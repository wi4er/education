...
import { Block2Permission } from '../entities/block/block2permission.entity';
...
export class BlockController {
  ...
  async findAll(): Promise<BlockView[]> {
  ...
  @CheckId(Block)
  @CheckMethodAccess(AccessEntity.BLOCK, AccessMethod.GET)
  async findOne(@Param('id') id: string): Promise<BlockView> {
  ...
  async create(
    ...
    const { strings, points, permissions, descriptions, ...blockData } = data;
    ...
    const block = await this.dataSource.transaction(async transaction => {
      ...
      await this.permissionAttributeService.create<Block>(transaction, Block2Permission, permissions);
  ...
  async update(
    ...
    const block = await this.dataSource.transaction(async transaction => {
      ...
      await this.permissionAttributeService.update<Block>(transaction, Block2Permission, id, permissions);
