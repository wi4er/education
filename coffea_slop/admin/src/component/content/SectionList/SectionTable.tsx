...
import { getDescriptionValue } from '../../../service/description.service';
...
export function SectionTable({
  ...
  function getActions(row: SectionView) {
    return [
      {
        title: 'Edit',
      ...
      ...statuses.map(status => ({
        ...
        onClick: () => {
          ...
          patchItem<SectionView>('section', row.id, { status: newStatuses })
            .then(() => onStatusChange?.());
        },
      })),
    ];
            ...
            <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
              ...
              {columns.map(column => {
                ...
                if (column.id.startsWith('str:')) {
                  ...
                  const value = row[column.id as keyof SectionView];
